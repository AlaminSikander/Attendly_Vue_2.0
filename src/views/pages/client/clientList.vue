...
const loading1 = ref(true);  // Initialize loading to true
const loading2 = ref(true);  // Initialize loading to true
...
onBeforeMount(() => {
    productService.getProductsWithOrdersSmall().then((data) => {
        products.value = data;
        loading2.value = false;  // Stop loading when data is fetched
    });
    ...
    customerService.getClientList().then((data) => {
        ...
        loading1.value = false;  // Stop loading when data is fetched
    });
                ...
                <DataTable :value="customer1" :paginator="true" :rows="10" dataKey="id" :rowHover="true"
                    ...
                    <!-- <Column header="Action" bodyClass="text-center" style="min-width: 8rem">
                        <template #body>
                            ...
                            <router-link :to="{ name: 'clientUpdate', params: { slug: client.slug } }"  class="text-light"> 
                                <Button title="Update Client" icon="pi pi-cog" severity="success" rounded outlined
                                    class="" />
                            </router-link>
                        </template>
                    </Column> -->
                    <Column header="Action" bodyClass="text-center" style="min-width: 8rem">
                        <template #body="{ data }">
                            ...
                            <router-link :to="{ name: 'clientUpdate', params: { slug: data.slug } }" class="text-light">
                                <Button title="Update Client" icon="pi pi-cog" severity="success" rounded outlined
                                    class="" />
                            </router-link>
                        </template>
                    ...
                    <Column header="Bulk Actions" bodyClass="text-center" style="min-width: 8rem">
                        <template #body>
                            <router-link :to="{ name: 'newClient' }" class="text-light">
                                <Button title="Bulk Actions" icon="pi pi-table" severity="success" rounded outlined
                                    class="" />
                            </router-link>
                        </template>
                    </Column>
